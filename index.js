...
function getKey(keyboardEvent) {
  if (keyboardEvent.type === 'keypress')
    return String.fromCharCode(keyboardEvent.charCode);
  var key = '';
...
function main({ dom, keyboard }) {
  ...
  const keyDown$ = keyboard.down$.map(e => `${getKey(e)} key is down`);
  const keyPress$ = keyboard.press$.map(e => `${getKey(e)} key is pressed`);
  const keyUp$ = keyboard.up$.map(e => `${getKey(e)} key is up`);
